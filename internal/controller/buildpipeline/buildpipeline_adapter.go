...
func (a *Adapter) EnsureSnapshotExists() (result controller.OperationResult, err error) {
	...
	if !h.HasPipelineRunSucceeded(a.pipelineRun) {
		a.handleUnsuccessfulPipelineRun(&canRemoveFinalizer, a.pipelineRun)
...
func (a *Adapter) handleUnsuccessfulPipelineRun(canRemoveFinalizer *bool, pipelineRun *tektonv1.PipelineRun) {
	// The build pipelinerun has not finished
	if h.HasPipelineRunFinished(pipelineRun) || pipelineRun.GetDeletionTimestamp() != nil {
		...
		a.logger.Info("Finished processing of unsuccessful build PLR",
			"statusCondition", pipelineRun.GetStatusCondition(),
			"deletionTimestamp", pipelineRun.GetDeletionTimestamp(),
		)
		*canRemoveFinalizer = true
	}
}
